...
int main( int argc, char **argv ) {
  ...
  if (argc != 3) {
    std::cout << "Syntax is $: mcmc inputfile outputfile" << std::endl;
    exit(-1);
  }
  ...
  double currentl = 0, steptime, currentx = 12000*rand.Uniform()-6000, currenty = 12000*rand.Uniform()-6000, currentz = 12000*rand.Uniform()-6000, currentt = 100*rand.Uniform()+fit_time-50, accrate = 0;
  int stepnum;
  bool accepted;
  ...
  for(size_t istep = 0; istep < numsteps; istep++){
    ...
    double propt = currentt + rand.Gaus(0, tsig);
    TVector3 propPos(propx, propy, propz);

    while( propPos.Mag() > 6000 || propt > fit_time + 100 || propt < fit_time - 100  ) {
      propl = 0;
      propx = currentx + rand.Gaus(0, xsig);
      propy = currenty + rand.Gaus(0, ysig);
      propz = currentz + rand.Gaus(0, zsig);
      propt = currentt + rand.Gaus(0, tsig);
	...
	double emisst2 = timeResCalc.CalcTimeResidual(pmt.GetID(), pmt.GetTime(), propPos, propt, false, 3.103125 * 1e-6, true, 0.0);
