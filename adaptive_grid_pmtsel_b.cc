...
int main( int argc, char **argv ) {
  ...
  CubeCollection init_cube_col;// = neCubeCollection();
  ...
  for( int i_cube = 0; i_cube < init_cube_col.GetNCubes(); i_cube++){
    delete init_cube_col.GetCube(i_cube);
  }
  ...
  for( int i_cube = 0; i_cube < col.GetNCubes(); i_cube++ ){
    ...
    for(int t = 0; t < num_t; t++){
      ...
      for( size_t i_pmt = 0; i_pmt < calibrated_PMTs.GetCount(); i_pmt++ ) {
	      ...
	      // Loop PMTs & get LLH
	      const RAT::DS::PMTCal pmt = calibrated_PMTs.GetPMT( i_pmt );
