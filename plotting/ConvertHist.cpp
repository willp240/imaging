...
void ConvertHist(TString fname){
  ...
  for(int cube=0; cube<12; cube++){
    ...
    double bestllh = 0;
    for(int i=1; i<=h_llh->GetXaxis()->GetNbins()/10; i++){
      for(int j=1; j<=h_llh->GetYaxis()->GetNbins()/10; j++){
	for(int k=cube*10; k<(cube*10 +10); k++){
	  double llh = h_llh->GetBinContent(i*10 +6, j*10 +6, (k+1)*10 +6);
	  //std::cout << llh << " in " << i*10 +6 << " " << j*10 +6 << " " << (k+1)*10 +6 << std::endl;
	  if(llh>bestllh)
	    bestllh=llh;
	}
      }
    ...
    for(int i=0; i<h_t->GetXaxis()->GetNbins()/10; i++){
      for(int j=0; j<h_t->GetXaxis()->GetNbins()/10; j++){
        for(int k=cube*10; k<(cube*10 +10); k++){
	  double llh = h_llh->GetBinContent(i*10 +6, j*10 +6, (k+1)*10 +6);
	  double t = h_t->GetBinContent(i*10 +6, j*10 +6, (k+1)*10 +6);
	  if(llh>bestllh*0.99){
            new_llh->SetBinContent(55+i, 55+j, k+1, llh);
	    new_t->SetBinContent(55+i, 55+j, k+1, t);
