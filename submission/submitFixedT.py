...
def pycondor_submit(exe_name, base_name, job_name, i, img_dir, input_file, over_dir, sleep_time = 1, priority = 5):
    '''
    ...
    out_macro_text = "#!/usr/bin/sh  \n" + \
                     ...
                     str(exec_path) + " " + os.path.abspath(input_file) + " " + os.path.abspath(over_dir) + "/" + str(job_name) + ".root " + str(i) + " \n"
...
if __name__ == "__main__":
    ...
    for i in range(236, 316, 1):

        job_id = "{0}_{1}".format(base_name,i)
        pycondor_submit(exe_name, base_name, job_id, i, img_dir, input_file, over_dir, sleep_time = 1, priority = 5)
